...
```html
<canvas id="tutorial" width="150" height="150"> Fallback content here. </canvas>
```
...
Given an image, this function takes the area of the source image specified by the rectangle whose _top-left_ corner is (sx, sy) and whose _width_ and _height_ are _sWidth_ and _sHeight_ and draws it into the canvas, placing it on the canvas at (dx, dy) and scaling it to the size specified by _dWidth_ and _dHeight_.
